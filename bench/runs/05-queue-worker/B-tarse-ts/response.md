Async queue worker.
`process(raw, store, write)`: validate vs `Job` (untrusted), skip if `store.seen(key)` (idempotent), else `await write(job)`.
Write ok → `ok`; failure → `retry` (`timeout`/`503`) else `failed`. Duplicate key → no write.
Invalid payload → reject with JSON path (`$.amount expected num`).
