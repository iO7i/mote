...
`load(raw)`: validate row vs `Row {id:num,name:str,deleted_at:string|null}`, map to `User`; `active = deleted_at === null`.
Bad `id` / null `name` → error with JSON path.
`page(total,size,n)` → `{ page: n, pages: Math.ceil(total/size), offset: n*size }`.
