...
const data = {
  ...
  pilot: { tasks: review.summary?.accepted ?? 0, totalTasks: review.tasks?.length ?? 0, equivalence: review.summary?.equivalencePass ?? 0, mutationControls: review.summary?.mutationControlPass ?? 0, familiarityRisk: review.summary?.highFamiliarityRisk ?? 0 },
...
<section class="panel"><h2>Pilot controls</h2>${bar("accepted tasks", data.pilot.tasks, data.pilot.totalTasks)}${bar("reference equivalence", data.pilot.equivalence, data.pilot.totalTasks)}${bar("mutation controls", data.pilot.mutationControls, data.pilot.totalTasks)}${bar("high familiarity risk", data.pilot.familiarityRisk, data.pilot.totalTasks)}<p><span class="pill warn">${data.fake.status}</span> fake provider is harness-only</p></section></div>
